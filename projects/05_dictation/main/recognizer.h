...
/*
 * MX-1: print the running tally of what raw_string has contained. Called from the
 * heartbeat and by the serial 'v' command. See probe_log_raw() in recognizer.c.
 */
void recognizer_probe_summary(void);

/* True while the AFE's neural VAD says someone is speaking. Drives the state chip. */
