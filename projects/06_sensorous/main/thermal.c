...
#define WARM_C    ((float)CONFIG_SENSOROUS_THERMAL_WARM_C)
#define HOT_C     ((float)CONFIG_SENSOROUS_THERMAL_HOT_C)
#define TRIP_C    ((float)CONFIG_SENSOROUS_THERMAL_TRIP_C)
#define HYST_C    ((float)CONFIG_SENSOROUS_THERMAL_HYST_C)
...
void thermal_init(void)
{
    ...
    ESP_LOGI(TAG, "guard on: chip %.1f C, PMU %.1f C; warm %d, hot %d, trip %d C on the hotter die, %d s to change level, %d C hysteresis",
             st.chip_c, st.pmu_c, CONFIG_SENSOROUS_THERMAL_WARM_C, CONFIG_SENSOROUS_THERMAL_HOT_C, CONFIG_SENSOROUS_THERMAL_TRIP_C,
             CONFIG_SENSOROUS_THERMAL_SAMPLES, CONFIG_SENSOROUS_THERMAL_HYST_C);
}
...
bool thermal_poll(thermal_status_t *out)
{
    ...
    if (!isnan(board)) {
        ...
        if (want != st.level) {
            ...
            if (s_agree >= CONFIG_SENSOROUS_THERMAL_SAMPLES) {
                st.level = want;
                s_agree = 0;
                changed = true;
            }
