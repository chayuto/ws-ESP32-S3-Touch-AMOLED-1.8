 ...
 * CONFIG_SENSOROUS_POWER_LOG_S seconds and on every change. Returns true on a change.
 */
bool pmu_poll(void);

/*
 * The app composes the periodic state record (it knows the screen, the heap, the
 * recogniser). pmu_poll() calls this every CONFIG_SENSOROUS_POWER_LOG_S seconds and
