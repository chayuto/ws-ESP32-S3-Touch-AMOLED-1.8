...
{
    struct tm copy = *t;
    setenv("TZ", is_utc ? "UTC0" : CONFIG_SENSOROUS_TZ, 1);
    tzset();
    copy.tm_isdst = -1;
    time_t secs = mktime(&copy);
    struct timeval tv = {.tv_sec = secs};
    settimeofday(&tv, NULL);
    setenv("TZ", CONFIG_SENSOROUS_TZ, 1);
    tzset();
    char buf[32];
...
static bool from_ntp(void)
{
    ...
    if (!wifi_sta_join(CONFIG_SENSOROUS_NTP_TIMEOUT_S)) {
        s_info.ntp_ms = (int)((esp_timer_get_time() - t_start) / 1000);
        return false;
    }
    bool ok = false;
    esp_sntp_config_t sc = ESP_NETIF_SNTP_DEFAULT_CONFIG(CONFIG_SENSOROUS_NTP_SERVER);
    esp_netif_sntp_init(&sc);
    if (esp_netif_sntp_sync_wait(pdMS_TO_TICKS(10000)) == ESP_OK) {
        setenv("TZ", CONFIG_SENSOROUS_TZ, 1);
        tzset();
        char buf[32];
        ESP_LOGI(TAG, "clock set from NTP (%s): %s", CONFIG_SENSOROUS_NTP_SERVER, timesync_now_str(buf, sizeof(buf)));
        s_info.clock = "ntp";
        if (rtc_ok) {
    ...
    /* Disassociate but leave the driver up: in this project wifiscan.c wants it
     * resident for the whole run, so tearing it down here would only mean
