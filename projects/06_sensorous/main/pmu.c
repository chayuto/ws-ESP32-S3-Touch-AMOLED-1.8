...
static int s_record_period_s = CONFIG_SENSOROUS_POWER_LOG_S;
...
esp_err_t pmu_init(void)
{
    ...
    uint8_t treg = 0;
    rd(REG_TREG, &treg);
    uint8_t want = CONFIG_SENSOROUS_PMU_TREG_C >= 120 ? 3 : CONFIG_SENSOROUS_PMU_TREG_C >= 100 ? 2 : CONFIG_SENSOROUS_PMU_TREG_C >= 80 ? 1 : 0;
    if ((treg & 0x03) != want) {
...
{
    s_record_period_s = seconds > 0 ? seconds : CONFIG_SENSOROUS_POWER_LOG_S;
}
