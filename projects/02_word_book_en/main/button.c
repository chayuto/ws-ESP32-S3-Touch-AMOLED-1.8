...
#define LONG_PRESS_US    ((int64_t)CONFIG_WORDBOOK_LONG_PRESS_MS * 1000)
...
static void sample(void *arg)
{
    ...
    if (s_down && !s_long_fired && esp_timer_get_time() - s_down_since >= LONG_PRESS_US) {
        s_long_fired = true;
        latch(BUTTON_LONG, (uint32_t)CONFIG_WORDBOOK_LONG_PRESS_MS);
    }
}
...
void button_init(void)
{
    ...
    ESP_LOGI(TAG, "BOOT button on GPIO %d, sampled every %d ms, long press %d ms; level now %d", BUTTON_GPIO,
             SAMPLE_US / 1000, CONFIG_WORDBOOK_LONG_PRESS_MS, gpio_get_level(BUTTON_GPIO));
}
