 ...
 *
 *   warm  CONFIG_WORDBOOK_THERMAL_WARM_C   the app dims the screen and says so
 *   hot   CONFIG_WORDBOOK_THERMAL_HOT_C    the app sleeps (screen off, recogniser stopped),
 *                                          stops charging, and BOOT no longer wakes it
 *   trip  CONFIG_WORDBOOK_THERMAL_TRIP_C   the app powers the board off through the PMU
 *
 * A level changes only after CONFIG_WORDBOOK_THERMAL_SAMPLES consecutive seconds agree,
 * and drops only CONFIG_WORDBOOK_THERMAL_HYST_C below the line it crossed.
